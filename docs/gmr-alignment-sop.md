...
The existing `apply_patch` in `examples/gmr_alignment_agent.py` does not
enforce this — if the VLM's patch omits `ik_match_table2`, the tables drift.
When writing a patch, always include both table entries.
