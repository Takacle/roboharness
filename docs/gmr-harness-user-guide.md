...
GMR-Harness 将 General Motion Retargeting (GMR) 与 roboharness 的对齐工具链集成，提供从注册新机器人到完成姿态校准的一站式流程：
...
1. [环境准备](#1-环境准备)
2. [新机器人注册](#2-新机器人注册)
3. [SMPL-X 模板校准](#3-smpl-x-模板校准)
4. [T-pose 规格制作](#4-t-pose-规格制作)
5. [偏移求解与数值验证](#5-偏移求解与数值验证)
6. [VLM 迭代优化](#6-vlm-迭代优化)
7. [完整流程示例](#7-完整流程示例)
8. [常见问题](#8-常见问题)
9. [命令速查](#9-命令速查)
...
### SMPL-X 源（identity root quaternion）
...
| SMPL-X 配置缺少 `world_rotation` 或使用旧的 legacy base rotation | 重新执行 `setup_robot.py --src smplx --auto_register --update_scripts` |
